...
public class RSEANGenerator: RSAbstractCodeGenerator {
    ...
    override public func isValid(contents: String) -> Bool {
        if super.isValid(contents) && self.length == contents.stringLength() {
            var sum_odd = 0
            var sum_even = 0
            ...
            return Int(contents[contents.stringLength() - 1]) == checkDigit
        }
        return false
    }
    ...
    override public func barcode(contents: String) -> String {
        ...
        if self.length == 13 {
            ...
            newContents = contents.substring(1, length: contents.stringLength() - 1)
        }
        
        var barcode = ""
        for i in 0..<newContents.stringLength() {
            let digit = Int(newContents[i])!
            if i < lefthandParity.stringLength() {
                barcode += self.parityEncodingTable[digit][lefthandParity[i]]!
                if i == lefthandParity.stringLength() - 1 {
