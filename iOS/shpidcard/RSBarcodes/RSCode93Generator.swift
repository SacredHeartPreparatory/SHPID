...
public class RSCode93Generator: RSAbstractCodeGenerator, RSCheckDigitGenerator {
    ...
    override public func isValid(contents: String) -> Bool {
        if contents.stringLength() > 0 && contents == contents.uppercaseString {
            for i in 0..<contents.stringLength() {
    ...
    public func checkDigit(contents: String) -> String {
        ...
        var sum = 0
        for i in 0..<contents.stringLength() {
            let character = contents[contents.stringLength() - i - 1]
        ...
        for i in 0..<newContents.stringLength() {
            let character = newContents[newContents.stringLength() - i - 1]
            let characterValue = CODE93_ALPHABET_STRING.location(character)
