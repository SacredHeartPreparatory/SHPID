...
public class RSUPCEGenerator: RSAbstractCodeGenerator, RSCheckDigitGenerator {
    ...
    func convert2UPC_A(contents:String) -> String {
        let code = contents.substring(1, length: contents.stringLength() - 2)
        let lastDigit = Int(code[code.stringLength() - 1])!
    ...
    override public func isValid(contents: String) -> Bool {
        return super.isValid(contents)
            && contents.stringLength() == 8
            && Int(contents[0])! == 0
            && contents[contents.stringLength() - 1] == self.checkDigit(contents)
    }
    ...
    override public func barcode(contents: String) -> String {
        let checkValue = Int(contents[contents.stringLength() - 1])!
        let sequence = UPCE_SEQUENCES[checkValue]
        var barcode = ""
        for i in 1..<contents.stringLength() - 1 {
